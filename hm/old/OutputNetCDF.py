...
class OutputNetCDF(object):
    ...
    def set_netcdf_format_options(self, netcdf_attr):
        ...
        self.zlib = False
        if 'formatNetCDF' in list(netcdf_attr.keys()):
            self.format = str(netcdf_attr['formatNetCDF'])
        if 'zlib' in list(netcdf_attr.keys()):
    ...
    def set_netcdf_y_orientation(self, netcdf_attr):        
        ...
        if 'netcdf_y_orientation_follow_cf_convention' in list(netcdf_attr.keys()):
    ...
    def create_netCDF(self, ncFileName, varname, dimensions=None):
        ...
        for item in varname:
            self.add_variable(
                ...
                item,
                zlib=self.zlib,
                fill_value=file_handling.missing_value)
